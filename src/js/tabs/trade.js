(function(module) {
    ...
    TradeTab.prototype.angular = function(module) {
        ...
        function TradeCtrl(books, $scope, id, network,
            ...
            $scope.currencies_all = Options.currencies_all; //require('../data/currencies');
            $scope.currencies = [];
            ...
            // Details for an order that is edited and sent to iCloud for modification
            $scope.editOrder = {
                seq: null,
                type: null,
                oldPrice: null,
            ...
            function scrollToMessages() {
                ...
                if (jqElem[0]) $timeout(function() {
                    jqElem[0].scrollIntoView(true);
                });
            }

            // Format a iCloud Amount for editing by the user
            function formatForEdit(amount) {
                var formatted = $filter('rpamount')(amount, {
                    group_sep: false,
                    hard_precision: true,
                    max_sig_digits: 20
                });
            ...
            // Create a iCloud Amount from primitives
            function createAmount(value, currency, issuer) {
                var ccy = currency || Currency.from_json('ICC');
                var formatted = '' + value + ' ' + (ccy.has_interest() ? ccy.to_hex() : ccy.get_iso());

                var amount = Amount.from_human(formatted, {
                    reference_date: new Date(+new Date() + REF_DATE_OFFSET)
                });
            ...
            $scope.beginEditOrder = function() {
                ...
                $scope.editOrder.oldPrice = Amount.from_json(myOrder.second).ratio_human(myOrder.first, {
            ...
            $scope.reset = function() {
                ...
                var pair = $scope.pairs_all[0].name;
            ...
            $scope.goto_order_currency = function() {
                ...
                var first = order.first_currency == 'ICC' ? 'ICC' : order.first_currency + '.' + order.first_issuer;

                var second = order.second_currency == 'ICC' ? 'ICC' : order.second_currency + '.' + order.second_issuer;
            ...
            function qtyChangedOnDeleted(result) {
                ...
                if (changedQty && changedQty.is_positive()) {
                    ...
                    popupScope.order = {
                        qtyChangeAfterCancel: changedQty
                    };
            ...
            $scope.update_first = function(type) {
                ...
                var first_currency = $scope.order.first_currency || Currency.from_json('ICC');
            ...
            $scope.update_price = function(type) {
                var order = $scope.order[type];
                var second_currency = $scope.order.second_currency || Currency.from_json('ICC');
            ...
            $scope.calc_second = function(type) {
                ...
                if (order.price_amount && order.price_amount.is_valid() &&
                    ...
                    order.second = +order.second_amount.to_human({
                        group_sep: false
                    });
                }
            };
            ...
            $scope.calc_first = function(type) {
                ...
                if (order.price_amount && order.price_amount.is_valid() &&
                    ...
                    if (!order.price_amount.is_native()) {
                        ...
                        order.first_amount = Amount.from_json(order.second_amount.to_text_full()).ratio_human(Amount.from_json(price + '/' + currency + '/' + issuer), {
                            reference_date: new Date()
                        });
                    } else {
                        order.first_amount = Amount.from_json(order.second_amount.to_text_full()).ratio_human(Amount.from_json(order.price_amount.to_text()), {
                            reference_date: new Date()
                        });
                    }
                    order.first = +order.first_amount.to_human({
                        group_sep: false
                    });
                }
            };
            ...
            $scope.$watch('first_currency_selected', function() {
                ...
                if ($scope.first_currency_selected == 'ICC') {
            ...
            $scope.$watch('second_currency_selected', function() {
                ...
                if ($scope.second_currency_selected == 'ICC') {
            ...
            $scope.add_pair = function() {
                // var formattedIssuerFirst = $scope.first_currency_selected === 'ICC' ? '' : '.' + $scope.first_issuer_selected;
                // var formattedIssuerSecond = $scope.second_currency_selected === 'ICC' ? '' : '.' + $scope.second_issuer_selected;
                ...
                $scope.order.currency_pair = $scope.first_currency_selected + '/' + $scope.second_currency_selected;

                $scope.userBlob.unshift('/trade_currency_pairs', {
            ...
            function updateSettings() {
                ...
                if (pair.length != 2 || pair[0].length === 0 || pair[1].length === 0) {
                    order.first_currency = Currency.from_json('ICC');
                    order.second_currency = Currency.from_json('ICC');
                ...
                if (first_currency.is_native()) {
                    ...
                    order.first_issuer = Options.gateway.address;
                }

                if (second_currency.is_native()) {
                    order.second_issuer = '';
                } else {
                    order.second_issuer = Options.gateway.address;
                }
                ...
                // Invalid issuers or ICC/ICC pair
            ...
            $scope.toggleOffers = function() {
                ...
                $scope.userBlob.set('/clients/rippletradecom/tradeoffers', {
                    hideOffers: $scope.hideOffers,
                    hideOrderBook: $scope.hideOrderBook
                });
            };
            ...
            var updateCanBuySell = function() {
                ...
                var canBuy = second_currency.is_native() ||
                    ...
                    ($scope.lines[second_issuer + ($scope.order.second_currency.has_interest() ? $scope.order.second_currency.to_hex() : $scope.order.second_currency.to_json())] && $scope.lines[second_issuer + ($scope.order.second_currency.has_interest() ? $scope.order.second_currency.to_hex() : $scope.order.second_currency.to_json())].balance.is_positive());

                var canSell = first_currency.is_native() ||
                    first_issuer == $scope.address ||
                    ($scope.lines[first_issuer + ($scope.order.first_currency.has_interest() ? $scope.order.first_currency.to_hex() : $scope.order.first_currency.to_json())] && $scope.lines[first_issuer + ($scope.order.first_currency.has_interest() ? $scope.order.first_currency.to_hex() : $scope.order.first_currency.to_json())].balance.is_positive());
            ...
            if ($routeParams.first && $routeParams.second) {
                ...
                ['first', 'second'].forEach(function(prefix) {
                    ...
                    param_is_valid[prefix] = !!routeCurrencies[prefix] && (routeCurrencies[prefix][1] === 'ICC' || !!routeIssuers[prefix]);
                });

                if (param_is_valid.first && param_is_valid.second && routeCurrencies.first[1] !== routeCurrencies.second[1]) {
                    currencyPairChangedByNonUser = true;
                    var formattedIssuerFirst = routeCurrencies.first[1] === 'ICC' ? '' : '.' + routeIssuers.first[1];
                    var formattedIssuerSecond = routeCurrencies.second[1] === 'ICC' ? '' : '.' + routeIssuers.second[1];
