...
SendTab.prototype.angular = function(module) {
    module.controller('SendCtrl', ['$scope', '$timeout', '$routeParams', 'rpId',
        ...
        function($scope, $timeout, $routeParams, id,
            ...
            var timer;
            var xrpCurrency = Currency.from_json('ICC');

            $scope.xrp = {
                name: xrpCurrency.to_human({
                    full_name: $scope.currencies_all_keyed.ICC.name
                }),
            ...
            $scope.check_dt_visibility = function() {
                ...
                send.show_dt_field =
                    ($routeParams.dt || send.dt || (send.recipient_info &&
                        'object' === typeof send.recipient_info &&
                        send.recipient_info.dest_tag_required)) && (!send.federation || (send.federation_record && !send.federation_record.dt));
            };
            ...
            $scope.update_destination = function() {
                ...
                // Trying to send ICC to self.
                // This is used to disable 'Send ICC' button
                send.self = recipient === $scope.address;

                // Trying to send to a iCloud name
            ...
            $scope.update_destination_remote = function() {
                ...
                if (send.federation) {
                    ...
                    id.resolveName(strippedRecipient, {
                        tilde: true
                    }).then(function(name) {
            ...
            // Check destination for ICC sufficiency and flags
            $scope.check_destination = function() {
                ...
                account.entry(function(e, data) {
                    $scope.$apply(function() {
                        ...
                        // If we get this far, we have a iCloud address resolved
                        send.recipient_resolved = true;
            ...
            $scope.update_currency_constraints = function() {
                ...
                // If we don't have information about the recipient iCloud account yet,
                ...
                if (send.recipient_info.exists) {
                    ...
                    network.remote.requestAccountCurrencies({
                        ...
                        .on('success', function(data) {
                            $scope.$apply(function() {
                                if (data.receive_currencies &&
                                    ...
                                    // add ICC if it's allowed
                                    if (!$scope.send.recipient_info.disallow_xrp) {
                                        send.currency_choices_constraints.accountLines.unshift('ICC');
                                    }
                    ...
                    // If the account doesn't exist, we can only send ICC
                    send.currency_choices_constraints.accountLines = ['ICC'];
                }
            ...
            $scope.update_amount = function() {
                ...
                var amount = send.amount_feedback = ripple.Amount.from_human('' + send.amount + ' ' + matchedCurrency, {
                ...
                if (send.quote_url) {
                    ...
                    // Cannot make ICC payment if the sender does not have enough ICC
                    send.sender_insufficient_xrp = send.amount_feedback.is_native() && $scope.account.max_spend && $scope.account.max_spend.to_number() > 1 && $scope.account.max_spend.compareTo(send.amount_feedback) < 0;
            ...
            $scope.update_paths = function() {
                ...
                // wangch added 
                if ($scope.send.currency_code == 'ICC' && $scope.send.federation) {
                    $scope.send.path_status = 'done';
                    var alt = {};

                    alt.amount = send.amount_actual
                    ...
                    alt.rate = alt.amount.ratio_human(amount, {
                        reference_date: slightlyInFuture
                    });
                    // // alt.send_max = alt.amount.product_human(Amount.from_json('1.001'));
                    $scope.send.alternatives.push(alt);
                    return;
                }
                ...
                pf.on('update', function(upd) {
                    ...
                    $scope.$apply(function() {
                        ...
                        if ((!upd.alternatives || !upd.alternatives.length)) {
                            ...
                            console.log("go here", $scope.send.federation, $scope.send.currency_code);
                        } else {
                            var currencies = {};
                            ...
                            $scope.send.alternatives = _.map(upd.alternatives, function(raw, key) {
                                ...
                                var altamount = Amount.from_json(raw.source_amount);
                                alt.amount = altamount.clone(); // wangch fix: ratio_human midify the this, so here clone this
                                ...
                                alt.rate = altamount.ratio_human(amount, {
                                    reference_date: slightlyInFuture
                                });
                                alt.send_max = alt.amount.product_human(Amount.from_json('1.001'));
                                ...
                                } else if ($scope.send.currency_code === 'ICC') {
                                    currentAlternatives.push(alt);
                                }
