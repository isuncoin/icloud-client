...
BalanceTab.prototype.angular = function (module)
{
                                     ...
                                     function ($scope, id, network, $filter, $http, appManager)
  {
    ...
    // -What (if any) market value in terms of ICC each balance has, according to
    ...
    $scope.currenciesAll = Options.currencies_all;// require('../data/currencies');
    $scope.currencies = [];

    $scope.exchangeRates || ($scope.exchangeRates = {ICC: 1});

    $scope.firstCurrencySelected = 'ICC';
    ...
    ($scope.selectedValueMetric = store.get('balance')) || ($scope.selectedValueMetric = 'ICC');
    ...
    // Maintain a dictionary for the value of each 'currency:issuer' pair, denominated in ICC.
    // Fetch the data from RippleCharts, and refresh it whenever any non-ICC balances change.
    ...
    // Whenever the ICC balance changes, update the aggregate value, but no need to refresh exchangeRates.
    ...
    function updateAggregateValueDisplayed() {
      ...
      if (!metric) {
        $scope.selectedValueMetric = $scope.selectedValueMetricCode = 'ICC';
      ...
      if ($scope.selectedValueMetricCode === 'ICC') {
          ...
          _.forEach(event.effects, function(effect){
            switch (effect.type) {
              ...
              case 'trust_change_balance':
                currency = effect.amount.is_native() ? 'ICC' : (effect.currency + ':' + effect.counterparty);
    ...
    $scope.$watch('firstCurrencySelected', function () {
      ...
      if ($scope.firstCurrencySelected == 'ICC') {
    ...
    $scope.$watch('secondCurrencySelected', function () {
      ...
      if ($scope.secondCurrencySelected == 'ICC') {
