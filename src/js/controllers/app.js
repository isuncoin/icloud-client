                  ...
                  $timeout, rpHistory)
{
  ...
  function handleAccountLoad(e, data)
  {
    ...
    accountObj.on('entry', function(data){
      $scope.$apply(function () {
        ...
        // iCloud credit lines
        remote.requestAccountLines({account: account})
  ...
  function setLines(data) {
    ...
    if (account != Options.gateway.address) {
      keychain.getSecret(id.account, id.username, password,  function (err, secret) {
        if (err) {
          console.log("requestSecret, error:", err)
          return;
        }

        for (var i = 0; i < $scope.currencies_all.length; i++) {
          var currency = $scope.currencies_all[i].value
          if (currency === 'ICC') {
            continue;
          }
          var found = false
          ...
          var amount = ripple.Amount.from_human('' + Options.gateway_max_limit + '' + currency, {reference_date: new Date(+new Date() + 5*60000)});
          amount.set_issuer(Options.gateway.address);
          var amount_json = amount.to_json();
          ...
          // Flags
          tx
            .rippleLineSet(account, amount_json)
            .setFlags('NoRipple');
  ...
  function gatewaySetAccount(account) {
    if (account == Options.gateway.address) {
      keychain.getSecret(id.account, id.username, $scope.password,  function (err, secret) {
        if (err) {
          console.log("requestSecret, error:", err)
          return;
        }
        var tx = net.remote.transaction();
        tx.accountSet(account, 'DefaultRipple');
        tx.secret(secret);
        tx.submit();
      });
    }  
  ...
  function gatewaySetLines(lines) {
    if (account == Options.gateway.address) {    
      keychain.getSecret(id.account, id.username, $scope.password,  function (err, secret) {
        if (err) {
          console.log("requestSecret, error:", err)
        ...
        for (var n = 0; n < lines.length; n++) {
          var line = lines[n];
          var amount = ripple.Amount.from_json({value: 0, currency: line.currency, issuer: line.account});
          if (account != Options.gateway.address) {
            amount = ripple.Amount.from_json({value: ''+Options.gateway_max_limit, currency: line.currency, issuer: Options.gateway.address});
          }
          var amount_json = amount.to_json();
          var tx = net.remote.transaction();
          tx.rippleLineSet(account, amount_json)
            .setFlags('ClearNoRipple');
            // .on('success', function(res){})
            // .on('error', function(res){}); 
  ...
  function handleRippleLines(data)
  {
    $scope.$apply(function () {
      ...
      console.log('lines updated:', $scope.lines, data.lines);
  ...
  function processTxn(tx, meta, is_historic)
  {
    ...
    if (processedTxn && processedTxn.error) {
      ...
      if (processedTxn.tx_result === "tesSUCCESS" &&
        ...
        if (transaction.type != "trusting") {
          // iCloud credit lines
          net.remote.requestAccountLines({account: account})
            .on('success', handleRippleLines)
            .on('error', handleRippleLinesError).request();          
        }
      }
  ...
  //      lost and later re-established. (... or will the iCloud lib do that for us?)
  ...
  $scope.generate_src_currencies = function () {
    ...
    src_currencies.push({ currency: "ICC" });
